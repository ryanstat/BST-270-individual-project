...
data %>% group_by(PLAYER = player) %>% 
  mutate(weight = possessions/sum(possessions)) %>% 
  mutate(weighted_DRAYMOND = DRAYMOND*weight) %>% 
  summarise(`POSSESSIONS PLAYED` = round(sum(possessions)), `DRAYMOND RATING` = round(sum(weighted_DRAYMOND),2)) %>% 
  filter(`POSSESSIONS PLAYED` >= 10000) %>% 
  arrange(desc(`DRAYMOND RATING`)) %>% 
  mutate_at(vars(`DRAYMOND RATING`), ~if_else(. > 0, paste0("+", .), paste0(.))) %>% 
  DT::datatable()
```
...
```{r}
data %>% filter(season == 2019, possessions >= 2000) %>%
  select(PLAYER = player, `POSSESSIONS PLAYED` = possessions, `DRAYMOND RATING` = DRAYMOND) %>% 
  arrange(desc(`DRAYMOND RATING`)) %>% 
  mutate_at(vars(`DRAYMOND RATING`), ~round(., 2)) %>% mutate_at(vars(`POSSESSIONS PLAYED`), ~round(.)) %>% 
  mutate_at(vars(`DRAYMOND RATING`), ~if_else(. > 0, paste0("+", .), paste0(.))) %>% 
  DT::datatable()
```
